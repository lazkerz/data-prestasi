...
<div class="bg-gray-50 p-6">
    <div class="h-fit md:ml-40 md:justify-center md:items-center mt-0 mb-10 gap-2">
        ...
        <div class="flex justify-center mx-auto w-[450px]">
            <form action="{{ route('prestasi.update', $prestasi->id) }}" method="POST" enctype="multipart/form-data" class="w-full gap-3 p-5 bg-white rounded-lg shadow-lg">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea name="deskripsi_prestasi" id="deskripsi_prestasi"
                        class="mt-1 w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>{{ old('deskripsi_prestasi', $prestasi->deskripsi_prestasi) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <p class="text-sm mt-2">File saat ini: <a href="{{ asset('storage/' . $prestasi->file_prestasi) }}" class="text-indigo-600 hover:text-indigo-900" target="_blank">Lihat File</a></p>
                    @endif
                </div>
